...
it("should render phemium-push", async () => {
  ...
  expect(page.root).toEqualHtml(`
    <phemium-push>
      <mock:shadow-root>
        <div class="absolute cursor-pointer flex flex-col h-20 items-start justify-center notification--color pl-4 w-full" id="notificationBox">
...
it("should initialize firebase", async () => {
  ...
  expect(push.firebaseConfig).toBeTruthy();

  await push.initialize(phemiumConfig, push.firebaseConfig, null);
  expect(firebase.app.initializeApp).toHaveBeenCalledWith(push.firebaseConfig);
});
